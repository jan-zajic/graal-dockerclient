...
import net.jzajic.graalvm.client.LogMessage.Stream;
...
public class LogReader implements Closeable {
  ...
  public LogMessage nextMessage() throws IOException {
    ...
    header.position(0);
    ByteBuffer result = ByteBuffer.allocate(frame.length+n).put(header).put(frame);
    result.position(0);
    return new LogMessage(streamId, result);
  }
