...
import static net.jzajic.graalvm.IsStruct.struct;
...
import net.jzajic.graalvm.client.DockerClient.Signal;
...
import net.jzajic.graalvm.client.messages.swarm.ConfigBind;
import net.jzajic.graalvm.client.messages.swarm.ConfigCreateResponse;
import net.jzajic.graalvm.client.messages.swarm.ConfigFile;
...
import net.jzajic.graalvm.client.messages.swarm.Preference;
...
import net.jzajic.graalvm.client.messages.swarm.Spread;
...
import net.jzajic.graalvm.client.messages.swarm.Version;
...
public class DefaultDockerClientUnitTest {
  ...
  public void testUpdateNode() throws Exception {
    ...
    final NodeSpec updatedNodeSpec = NodeSpec.builder()
  ...
  public void testCreateServiceWithPlacementPreference()
    ...
    assertThat(service.spec.taskTemplate.placement, is(struct(Placement.class).where(taskSpec.placement)));
  }
  ...
  public void testBufferedRequestEntityProcessing() throws Exception {
    builder.requestEntityChunked(false);
    final DefaultDockerClient dockerClient = new DefaultDockerClient(builder);
  ...
  public void testChunkedRequestEntityProcessing() throws Exception {
    builder.requestEntityChunked(true);
    final DefaultDockerClient dockerClient = new DefaultDockerClient(builder);
  ...
  public void testInspectTask() throws Exception {
    ...
    assertThat(task, is(struct(Task.class)
        .where("id", is("0kzzo1i0y4jz6027t0k7aezc7"))
        .where("version", is(struct(Version.class)
            .where("index", is(71L))
        ))
        ...
        .where("spec", is(struct(TaskSpec.class)
            .where("containerSpec", is(struct(ContainerSpec.class)
                .where("image", is("redis"))
            ))
            .where("resources", is(struct(ResourceRequirements.class)
                .where("limits",
                    is(struct(net.jzajic.graalvm.client.messages.swarm.Resources.class)))
                .where("reservations",
                    is(struct(net.jzajic.graalvm.client.messages.swarm.Resources.class)))
            ))
        ))
  ...
  public void testListTaskWithCriteria() throws Exception {
    ...
    final JsonNode requestJson2 =
        toJson(urlDecode(recordedRequest2.getRequestUrl().queryParameter("filters")));
    ...
    final JsonNode requestJson3 =
        toJson(urlDecode(recordedRequest3.getRequestUrl().queryParameter("filters")));
  ...
  private void enqueueServerApiVersion(final String apiVersion) {
	...
	private String urlDecode(final String encoded) throws DockerException, UnsupportedEncodingException {
		final String decoded = URLDecoder.decode(encoded, StandardCharsets.UTF_8.name());
		return decoded;
	}
